        ...
        public CustomSchemaExtender()
        {
            ...
            ExtendTypes<ObjectGraphType<Item>>(type =>
            {
                ...
                type.Field<StringGraphType>("personapplicanturl",
                    ...
                    resolve: context => ResolvePersonApplicantUrl(context.Source));
            });


        }

        private string ResolvePersonApplicantUrl(Sitecore.Data.Items.Item personItem)
        {
            var urlOptions = new Sitecore.Links.UrlOptions() { ShortenUrls = true };
            var url = Sitecore.Links.LinkManager.GetItemUrl(personItem, urlOptions).Replace("/MvpSite/Home", "");
            return url;
        }
